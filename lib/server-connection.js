...
var serverConnection = function(socket, server, closeConnectionServerCallback, smppDefs) {
    ...
    self.socketDataHandler = function(buffer) {
		if(self.config.debug){ console.log("Got DATA", buffer.toString()); }
		
		if(buffer.toString()==="PING"){console.log("PONG", arguments); return;}
    ...
    self.check_enquire_link_status = function() {
        ...
        if (now - 15000 >= self.enquire_link_resp_received) {
            ...
            if ( self.config.debug ) { console.log("connection timed out"); }
            self.socket.destroy();
        }
    };
    ...
    self.socketErrorHandler = function(e) {
        switch(e.errno) {
            case 32:
                // Broken pipe
                if ( self.config.debug ) { console.log('unexpected client disconnect'); }
                self.socket.destroy();
                break;
            case 104:
                if ( self.config.debug ) { console.log('SOCKET ERROR (104): connection reset'); }
                self.socket.end();
                break;
            case 111:
                if ( self.config.debug ) { console.log('SOCKET ERROR (111): connection refused'); }
                self.socket.end();
                break;
            default:
                if ( self.config.debug ) { console.log('unknown socket error; closing socket'); }
                self.socket.destroy();
                break;
        }
