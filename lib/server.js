...
exports.server = function(config, smppDefs) {
    ...
    self.start = function() {
        ...
        self.server.listen(self.config.port, self.config.host, function() {
            if ( self.config.debug ) { console.log('listening on ' + self.config.host + ':' + self.config.port); }
        });
    ...
    self.connectionListener = function(incomingSocket) {
        if ( self.config.debug ) { console.log("Incoming Socket");}
		
		incomingSocket.on("connect", function(){console.log("connect");});
		incomingSocket.on("data", function(){console.log("data");});
		incomingSocket.on("end", function(){console.log("end");});
		incomingSocket.on("timeout", function(){console.log("timeout");});
		incomingSocket.on("drain", function(){console.log("drain");});
		incomingSocket.on("error", function(){console.log("error");});
		incomingSocket.on("close", function(){console.log("close");});
    ...
    self.serverErrorHandler = function(e) {
        if ( self.config.debug ) { console.log(sys.inspect(e)); }
    };

    self.clientCloseHandler = function(id) {
        if (self.config.debug) { console.log('connection closed', id); }
