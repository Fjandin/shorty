...
exports.client = function(config, smppDefs) {
    ...
    self.connect = function() {
		...
		self.socket.on("data", function(){console.log("data");});
		self.socket.on("end", function(){console.log("end");});
		self.socket.on("timeout", function(){console.log("timeout");});
		self.socket.on("drain", function(){console.log("drain");});
		self.socket.on("error", function(){console.log("error");});
		self.socket.on("close", function(){console.log("close");});
        ...
        self.socket.on('connect', function() {
            ...
            if(self.config.debug){ console.log("Connected: send bind in 5 seconds"); }
			setTimeout(function(){
				this.bind();
			}.bind(self), 5000);
        });
        ...
        self.socket.on('data', function(buffer) {
            if(self.config.debug){ console.log("Got Data", buffer); }
            ...
            for (i = 0; i < pdus.length; i++) {
				...
				self.emit("recievePdu", smpp.command_ids[pdu.command_id], pdu);
				
				if(self.config.debug){ console.log("Got PDU", smpp.command_ids[pdu.command_id], JSON.stringify(pdu)); }
                ...
                switch (pdu.command_id) {
                    ...
                    case smpp.commands.unbind_resp:
                        ...
                        break;
					case smpp.commands.query_sm_resp:
						self.emit("query_sm_resp", pdu);
                    default:
                        break;
                }
            }
    ...
    self.handleDeliverSm = function(pdu) {
        self.emit('deliver_sm', pdu, function(command_status){
			var newPdu = {
     ...
     */
    self.sendMessage = function(params, optional, sequence_number) {
        var pdu, success;
    ...
    self.sendPdu = function(pdu) {
		if (self.socket.readyState === 'open') {
            self.emit("sendPdu", pdu.command, pdu);
			self.socket.write(writer.write(pdu));
            if(self.config.debug){ console.log("sendPdu", pdu.command, "TRUE"); }
			return true;
        } else {
            self.socket.destroy();
			 if(self.config.debug){ console.log("sendPdu", pdu.command, "FALSE"); }
            return false;
        }
    };

    self.connectionClose = function(e) {
        if(self.config.debug){ console.log("Socket Close", e); }
    ...
    self.socketErrorHandler = function(e) {
        if(self.config.debug){ console.log("Socket Error", e); }
